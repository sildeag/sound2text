...
Write-Host "Fixing for Redux patterns correction..."

Get-ChildItem -Recurse -Filter *.kt | ForEach-Object {
    $path = $_.FullName
    $c = Get-Content $path -Raw

    # fix previous correction
    if ($c -match '// TODO: remove global singleton: object ')
    {
        $fixed = $c -replace '// TODO: remove global singleton: object ', 'object'
        Set-Content $path $fixed
        Write-Host "[REPAIRED] Restored object declaration in $path"
    }
}
