...
foreach ($file in $allFiles) {
    ...
    foreach ($keyword in $commonComposeImports.Keys) {
        ...
        # Regex: match keyword as a standalone identifier
        # \b ensures word boundaries so "Column" doesn't match "MyColumnWrapper"
        $pattern = "\b$keyword\b"
        ...
        if ($cleanContent -match $pattern) {
            ...
            # Check if the import is missing (regex again for robustness)
            $importPattern = [Regex]::Escape($targetImport)

            if ($compactContent -notmatch $importPattern) {
