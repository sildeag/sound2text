...
class NoteRepositorySQLite(
    ...
    private fun loadFields(noteId: String): List<NoteField> {
        ...
        connection.prepare(
            "SELECT id, type, level, anchor, language, payload FROM note_fields WHERE note_id = ?"
        ).use { stmt ->
    ...
    override suspend fun save(note: Note) {
        ...
        connection.prepare(
            "DELETE FROM note_fields WHERE note_id = ?"
        ).use { stmt ->
            stmt.bindText(1, note.id)
            stmt.step()
        }
        note.fields.forEach { field ->
            val (type, payload) = when (field) {
                is NoteTextField ->
                    "text" to NoteFieldPayload.Text(field.text, field.voiceEnabled)
                is NoteCheckboxField ->
                    "checkbox" to NoteFieldPayload.Checkbox(field.checked)
                is NoteDropdownField ->
                    "dropdown" to NoteFieldPayload.Dropdown(field.options,
                        field.selected)
                is NotePdfField ->
                    "pdf" to NoteFieldPayload.Pdf(field.pdfFieldName, field.value)
            }
            connection.prepare(
                """
 INSERT INTO note_fields (id, note_id, type, level, anchor,
language, payload)
 VALUES (?, ?, ?, ?, ?, ?, ?)
 """.trimIndent()
            ).use { stmt ->
